"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserModel = void 0;
var database_1 = __importDefault(require("../database"));
var bcrypt_1 = __importDefault(require("bcrypt"));
var UserModel = /** @class */ (function () {
    function UserModel() {
    }
    UserModel.prototype.update = function (userParam) {
        return __awaiter(this, void 0, void 0, function () {
            var sql, hash, values, Client, user, result, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'UPDATE users SET username = ($2), firstname = ($3), lastname = ($4), password = ($5) WHERE id=($1) RETURNING *';
                        hash = bcrypt_1.default.hashSync(userParam.password + process.env.BCRYPT_PASSWORD, parseInt(process.env.SALT_ROUNDS, 10));
                        values = [userParam.id, userParam.username, userParam.firstname, userParam.lastname, hash];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                                values: values,
                            })];
                    case 3:
                        result = _a.sent();
                        user = result.rows[0];
                        return [2 /*return*/, user];
                    case 4:
                        error_1 = _a.sent();
                        console.error("Error while inserting user:", error_1);
                        throw error_1;
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    UserModel.prototype.show = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var sql, values, Client, user, result, error_2;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'SELECT * FROM users WHERE id=($1)';
                        values = [id];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                                values: values,
                            })];
                    case 3:
                        result = _a.sent();
                        user = result.rows[0];
                        return [2 /*return*/, user];
                    case 4:
                        error_2 = _a.sent();
                        console.error("Error while inserting user:", error_2);
                        throw error_2;
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    UserModel.prototype.showAll = function () {
        return __awaiter(this, void 0, void 0, function () {
            var sql, Client, userList, result, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'SELECT * FROM users';
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                            })];
                    case 3:
                        result = _a.sent();
                        userList = result.rows;
                        return [2 /*return*/, userList];
                    case 4:
                        error_3 = _a.sent();
                        console.error("Error while inserting user:", error_3);
                        throw error_3;
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    UserModel.prototype.create = function (b) {
        return __awaiter(this, void 0, void 0, function () {
            var sql, hash, values, Client, result, error_4;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'INSERT INTO users (username, firstname, lastname, password) VALUES($1, $2, $3, $4) RETURNING *';
                        hash = bcrypt_1.default.hashSync((b.password).trim(), parseInt(process.env.SALT_ROUNDS, 10));
                        values = [b.username, b.firstname, b.lastname, hash];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                                values: values,
                            })];
                    case 3:
                        result = _a.sent();
                        return [2 /*return*/, result.rows[0]];
                    case 4:
                        error_4 = _a.sent();
                        console.error("Error while inserting user:", error_4);
                        throw error_4;
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    UserModel.prototype.delete = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var sql, values, Client, result, error_5;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'DELETE FROM users WHERE id=($1)';
                        values = [id];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                                values: values,
                            })];
                    case 3:
                        result = _a.sent();
                        return [2 /*return*/, 1];
                    case 4:
                        error_5 = _a.sent();
                        console.error("Error while inserting user:", error_5);
                        throw error_5;
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    UserModel.prototype.authenticate = function (username, password) {
        return __awaiter(this, void 0, void 0, function () {
            var sql, values, Client, rows, user, err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        sql = 'SELECT password FROM users WHERE username=($1)';
                        values = [username];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 4, 5, 6]);
                        return [4 /*yield*/, database_1.default.connect()];
                    case 2:
                        Client = _a.sent();
                        return [4 /*yield*/, Client.query({
                                text: sql,
                                values: values,
                            })];
                    case 3:
                        rows = (_a.sent()).rows;
                        if (rows.length > 0) {
                            user = rows[0];
                            if (bcrypt_1.default.compareSync((password).trim(), user.password)) {
                                return [2 /*return*/, user];
                            }
                        }
                        return [2 /*return*/, null];
                    case 4:
                        err_1 = _a.sent();
                        throw new Error("Could not find user ".concat(username, ". ").concat(err_1));
                    case 5:
                        if (Client) {
                            Client.release();
                        }
                        return [7 /*endfinally*/];
                    case 6: return [2 /*return*/];
                }
            });
        });
    };
    return UserModel;
}());
exports.UserModel = UserModel;
